...
#define IMAGE 0
#define IMAGE_STD 1
#define IMAGE_SRC "/home/chenyuan/3rdparty/tf-pose-estimation/images/p3.jpg"
...
//                }
            ...
            //显示heatmap
            Mat heatmaps = cv::Mat(36, 36, CV_32FC1, Scalar(0));
            ...
            Mat vectormap_x = cv::Mat(36, 36, CV_32FC1, Scalar(0));
            Mat vectormap_y = cv::Mat(36, 36, CV_32FC1, Scalar(0));
            int paf_c = 0;
...
//                std::cout << heatmap_peaks << std::endl;
                ...
                for (int y = 0; y < 36; y++) {
                    for (int x = 0; x < 36; x++) {
                        buffer_heatmap.get()[19 * (36 * y + x) + c] = heatmap.at<float>(y, x);
                        buffer_peaks.get()[19 * (36 * y + x) + c] = heatmap_peaks.at<float>(y, x);
                    }
                }
                c++;
            }
            ...
            for (Mat &paf : paf_channels) {
                ...
                for (int y = 0; y < 36; y++) {
                    for (int x = 0; x < 36; x++) {
                        buffer_pafmap.get()[38 * (36 * y + x) + c] = paf.at<float>(y, x);
                    }
                }
                c++;
            }

            process_paf(36, 36, 19, buffer_peaks.get(), 36, 36, 19, buffer_heatmap.get(), 36, 36, 38,
                        buffer_pafmap.get());
            ...
            while (num_humans > 0) {
                ...
                for (int part_idx = 0; part_idx < 18; part_idx++) {
                    ...
                    BodyPart bodyPart = BodyPart(part_idx, (float)(get_part_x(c_idx)/36.0), (float)(get_part_y(c_idx)/36.0), get_part_score(c_idx));
